...
export function CreateWidgetTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const [name, setName] = useState("Web Widget");
  ...
  const loadData = async (nextSelectedId?: string | null, preserveCreateMode = false) => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "위젯 목록을 불러오지 못했습니다.");
      setWidgets([]);
      setAgents([]);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    setName(selectedWidget.name || "Web Widget");
  ...
  const handleSave = async (rotateKey = false) => {
    ...
    try {
      ...
      toast.success(rotateKey ? "위젯 키가 재발급되었습니다." : "위젯이 저장되었습니다.");
      await loadData(response.item.id || null);
      setSelectedId(response.item.id || null);
      setIsCreating(false);
    } catch (saveError) {
      toast.error(saveError instanceof Error ? saveError.message : "위젯 저장에 실패했습니다.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (!isAdmin) {
      toast.error("위젯 삭제는 관리자만 가능합니다.");
      return;
    }
    if (!window.confirm("선택한 위젯을 삭제할까요?")) return;
    try {
      ...
      toast.error(deleteError instanceof Error ? deleteError.message : "위젯 삭제에 실패했습니다.");
    }
  };
  ...
  const banner = !isAdmin ? (
    <StateBanner
      tone="warning"
      title="관리자 전용 편집"
      description="위젯 생성, 저장, 삭제, 키 재발급은 관리자 권한이 필요합니다. 현재 화면은 읽기 전용으로 동작합니다."
    />
  ) : error ? (
    <StateBanner tone="danger" title="Widget 로딩 실패" description={error} />
  ) : null;

  return (
    <CreateResourceShell
      description="위젯 템플릿을 선택한 뒤 연결 에이전트와 테마를 수정하고, 설치 키와 스크립트를 바로 확인합니다."
      ...
      listContent={
        widgets.length === 0 && !loading ? (
          <div className="p-4 text-sm text-slate-500">생성된 위젯이 없습니다.</div>
        ) : (
          <CreateListTable
            ...
            columns={[
              {
                ...
                render: (item) => <div className="truncate text-sm font-semibold text-slate-900">{item.name || "Web Widget"}</div>,
              },
              {
                id: "agent",
                label: "에이전트",
      ...
      detailTitle={isCreating ? "새 위젯" : selectedWidget?.name || "위젯을 선택하세요"}
      detailDescription={
        isCreating
          ? "위젯 이름, 에이전트, 테마를 입력해 새 위젯을 생성합니다."
          : selectedWidget
            ? "연결 에이전트, 메시지, 설치 스크립트를 바로 조정할 수 있습니다."
            : "좌측 목록에서 위젯을 선택하면 상세 편집이 열립니다."
      }
      detailActions={
        ...
        <div className="space-y-4">
          {isCreating || selectedWidget ? (
            <>
              ...
              <label className="block">
                <div className="mb-1 text-xs text-slate-600">연결 에이전트</div>
                <SelectPopover
                  value={agentId}
                  onChange={setAgentId}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
                <div className="text-xs text-slate-500">
                  {isCreating ? "새 위젯을 생성합니다." : isDirty ? "저장되지 않은 변경 사항이 있습니다." : "변경 사항이 없습니다."}
                </div>
                <div className="flex items-center gap-2">
                  {isCreating ? (
                    <Button
                  ...
                  >
                    {saving ? "저장 중..." : isCreating ? "위젯 생성" : "저장"}
                  </Button>
                </div>
              </div>
            </>
            ...
            <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-6 text-sm text-slate-500">
              좌측 목록에서 위젯을 선택하거나 새 위젯을 생성해 주세요.
            </div>
          )}
        </div>
      }
