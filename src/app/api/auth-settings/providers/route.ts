...
export async function GET(req: NextRequest) {
  ...
  if (provider === "chat_policy") {
    return NextResponse.json({ error: "CHAT_POLICY_DEPRECATED" }, { status: 410 });
  }
  ...
  const providers = (result.data?.providers || {}) as Record<string, Record<string, unknown> | undefined>;
