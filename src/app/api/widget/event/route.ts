...
export async function POST(req: NextRequest) {
  ...
  const widgetId = readWidgetTokenInstanceId(payload) || readWidgetTokenTemplateId(payload) || payload.widget_id;
  ...
  const { error } = await supabaseAdmin.from("F_widget_events").insert({
    ...
    widget_id: widgetId,
    session_id: String(payload.session_id || "").trim() || null,
    event_type: String(body.type || "").trim(),
    payload: {
      ...(body.payload && typeof body.payload === "object" ? (body.payload as Record<string, unknown>) : {}),
      template_id: readWidgetTokenTemplateId(payload) || null,
      instance_id: readWidgetTokenInstanceId(payload) || null,
    },
    created_at: nowIso(),
  });
