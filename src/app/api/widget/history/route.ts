...
export async function GET(req: NextRequest) {
  ...
  const metadataTemplateId = metadata ? String(metadata.template_id || "").trim() : "";
  if (tokenInstanceId) {
    if (metadataWidgetId && metadataWidgetId !== tokenInstanceId) {
    ...
    if (tokenTemplateId && metadataTemplateId && metadataTemplateId !== tokenTemplateId) {
      return NextResponse.json({ error: "SESSION_WIDGET_MISMATCH" }, { status: 403 });
    }
  }
