...
export function cleanAddressCandidate(text: string) {
  ...
  if (!v) return "";
  // Drop leading address labels.
  v = v.replace(/^(?:주소|배송지|수령지)\s*[:\-]?\s*/g, "");
  // Drop trailing "변경/수정" hints.
  v = v.replace(/\s*(?:로|으로)?\s*(?:변경|수정|교체|바꿔)\s*(?:주세요|바랍니다|요)?[.!?~]*$/g, "");
  v = v.replace(/[,.!?~]+$/g, "").trim();
  return v;
}
...
export function hasKoreanAddressCue(text: string) {
  ...
  return /[가-힣]{2,}\s*(?:로|길|동|읍|면|리)(\s|$)/.test(v);
}
...
export function isLikelyAddressDetailOnly(text: string) {
  ...
  return /(?:[A-Za-z]?\d{1,5}(?:-[A-Za-z0-9]{1,5})?\s*(?:동|호|층|번지|관|실)|\b\d{1,5}\b)/.test(v);
}
...
export function extractAddressDetail(text: string) {
  ...
  const detailMatch = v.match(
    /((?:\d+\s*)?(?:[A-Za-z]?\d{1,5}(?:-[A-Za-z0-9]{1,5})?\s*(?:동|호|층|번지|관|실|\d{1,5}))$)/i
  );
  if (detailMatch) return normalizeAddressText(detailMatch[1]);
  const matches = Array.from(
    v.matchAll(/(\d+\s*[A-Za-z]?\d{1,5}(?:-[A-Za-z0-9]{1,5})?\s*(?:동|호|층|번지|관|실|\b\d{1,5}\b))/gi)
  );
  if (matches.length > 0) {
...
export function splitAddressForUpdate(
  ...
  if (chosenBase) {
    ...
    const hasExplicitUnitMarker =
      /(?:동|호|층|번지|관|실)$/i.test(extractedDetail) || /(?:동|호|층|번지|관|실)\b/i.test(raw);
...
export function extractChoiceIndex(text: string, max: number) {
  const match = text.match(/(?:^|\s)(\d{1,2})(?:\s*(?:번|번째))?/);
