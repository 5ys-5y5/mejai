...
const resolvedSet = new Set(["?¥Í≤∞", "Resolved", "resolved"]);
const escalatedSet = new Set(["?¥Í?", "Escalated", "escalated"]);
...
function buildChangeSummary(current: AgentItem, prev?: AgentItem | null) {
  ...
  if ((current.name || "") !== (prev.name || "")) changes.push("?¥Î¶Ñ");
  ...
  if ((current.website || "") !== (prev.website || "")) changes.push("?πÏÇ¨?¥Ìä∏");
  if ((current.goal || "") !== (prev.goal || "")) changes.push("Î™©Ìëú");
  return changes.length > 0 ? `${changes.join(", ")} Î≥ÄÍ≤? : "Î≥ÄÍ≤??ÜÏùå";
}

function formatDuration(seconds?: number | null) {
  const safe = Number(seconds || 0);
  if (!Number.isFinite(safe) || safe <= 0) return "0Î∂?;
  const minutes = Math.round(safe / 60);
  if (minutes < 60) return `${minutes}Î∂?;
  const hours = Math.floor(minutes / 60);
  const rest = minutes % 60;
  return rest > 0 ? `${hours}?úÍ∞Ñ ${rest}Î∂? : `${hours}?úÍ∞Ñ`;
}
...
export default function AgentDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!mounted) return;
        setError("?êÏù¥?ÑÌä∏Î•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??");
        setLoading(false);
      }
    }
    load();
  ...
  const kbInfoText = useMemo(() => {
    if (!selectedKb) return "?†ÌÉù??KBÍ∞Ä ?ÜÏäµ?àÎã§.";
    const adminFlag = isAdminKbValue(selectedKb.is_admin);
    return [
      `?úÎ™©: ${selectedKb.title || "-"}`,
      `Î≤ÑÏ†Ñ: ${selectedKb.version || "-"}`,
      `?ÅÌÉú: ${selectedKb.is_active ? "Î∞∞Ìè¨" : "ÎπÑÌôú??}`,
      `?†Ìòï: ${adminFlag ? "ADMIN" : "?ºÎ∞ò"}`,
      `?¥Ïö©:\n${selectedKb.content || "-"}`,
      `ID: ${selectedKb.id}`,
    ].join("\n");
  }, [selectedKb]);
  ...
  const kbOptions = useMemo(() => {
    return [...scopedKbItems]
      ...
      .map((item) => ({
        ...
        description: item.is_active ? "Î∞∞Ìè¨" : "ÎπÑÌôú??,
      }));
  }, [scopedKbItems]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("?êÏù¥?ÑÌä∏Í∞Ä ?Ä?•Îêò?àÏäµ?àÎã§.");
      ...
      const message = err instanceof Error ? err.message : "?êÏù¥?ÑÌä∏ ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§.";
      toast.error(message || "?êÏù¥?ÑÌä∏ ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleActivateVersion = async (id: string) => {
    ...
    try {
      ...
      toast.success("Î∞∞Ìè¨ Î≤ÑÏ†Ñ??Î≥ÄÍ≤ΩÎêò?àÏäµ?àÎã§.");
      await refreshAgents();
    } catch (err) {
      const message = err instanceof Error ? err.message : "Î∞∞Ìè¨ ?ÅÌÉú Î≥ÄÍ≤ΩÏóê ?§Ìå®?àÏäµ?àÎã§.";
      toast.error(message || "Î∞∞Ìè¨ ?ÅÌÉú Î≥ÄÍ≤ΩÏóê ?§Ìå®?àÏäµ?àÎã§.");
    } finally {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("??Î≤ÑÏ†Ñ????†ú?†Íπå?? ??†ú??Î≤ÑÏ†Ñ?Ä Î≥µÍµ¨?????ÜÏäµ?àÎã§.")) return;
    try {
      await apiFetch(`/api/agents/${id}`, { method: "DELETE" });
      const next = allAgents.filter((item) => item.id !== id);
      setAllAgents(next);
      toast.success("Î≤ÑÏ†Ñ????†ú?òÏóà?µÎãà??");
      if (id === agentId) {
        router.push("/app/agents");
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : "Î≤ÑÏ†Ñ ??†ú???§Ìå®?àÏäµ?àÎã§.";
      toast.error(message || "Î≤ÑÏ†Ñ ??†ú???§Ìå®?àÏäµ?àÎã§.");
    }
  };
    ...
    <div className="px-5 md:px-8 py-6">
      <div className="mx-auto w-full max-w-6xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">?êÏù¥?ÑÌä∏ ?§Ï†ï</h1>
            <p className="mt-1 text-sm text-slate-500">Î≤ÑÏ†ÑÎ≥?Íµ¨ÏÑ±Í≥??±Í≥ºÎ•??ïÏù∏?????àÏäµ?àÎã§.</p>
          </div>
        ...
        <Card className="mt-6 p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-slate-900">?êÏù¥?ÑÌä∏ Íµ¨ÏÑ±</div>
              <p className="mt-1 text-xs text-slate-500">
                ?ÑÏû¨ Î≤ÑÏ†Ñ {currentVersion || "-"} ¬∑ LLM/MCP/KB Î≥ÄÍ≤?????Î≤ÑÏ†Ñ???ùÏÑ±?©Îãà??
              </p>
            </div>
            <button
              type="button"
          ...
          {loading ? (
            <div className="mt-4 text-sm text-slate-500">?êÏù¥?ÑÌä∏Î•?Î∂àÎü¨?§Îäî Ï§?..</div>
          ) : error ? (
            <div className="mt-4 text-sm text-rose-600">{error}</div>
          ) : (
            <div className="mt-5 grid gap-5">
              <div className="grid gap-2">
                <label className="text-sm font-medium text-slate-900">?êÏù¥?ÑÌä∏ ?¥Î¶Ñ *</label>
                <input
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="?êÏù¥?ÑÌä∏ ?¥Î¶Ñ"
              ...
              <div className="grid gap-4">
                ...
                >
                  <label className="text-sm font-medium text-slate-900">KB Î≤ÑÏ†Ñ *</label>
                  <div className="flex items-center gap-2">
                    <SelectPopover
                      value={kbId}
                      ...
                      renderValue={(selected) => (
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 truncate">
                            ...
                            <span className="truncate">{selected?.label || "KB ?†ÌÉù"}</span>
                          </div>
                          {selected?.id ? (
                  ...
                  {kbUpdateAvailable ? (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
                      ÏµúÏã† KB Î≤ÑÏ†Ñ???àÏäµ?àÎã§ ({selectedKb?.version || "-"} ??{activeKb?.version || "-"}).
                      <button
                        type="button"
                        onClick={() => {
                  ...
                  {isAdmin ? (
                    <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
                        ...
                        <div className="mt-2 text-[11px] text-slate-500">
                          Admin KB??DB ?∏Î¶¨Í±∞Î°ú ?êÎèô Í¥ÄÎ¶¨Îê©?àÎã§. ?¨Í∏∞?úÎäî Î≥ÄÍ≤ΩÌï† ???ÜÏäµ?àÎã§.
                        </div>
                        {assignedAdminKbItems.length > 0 ? (
                          ...
                          <div className="mt-2 text-[11px] text-slate-500">?ÅÏö©??admin KBÍ∞Ä ?ÜÏäµ?àÎã§.</div>
                        )}
                    </div>
                  ) : null}
                </div>
              ...
              >
                <label className="text-sm font-medium text-slate-900">MCP ?ÑÍµ¨</label>
                <div className="flex items-center gap-2">
                  <MultiSelectPopover
                    values={mcpToolIds}
                    onChange={handleMcpToolIdsChange}
                    options={mcpOptions}
                    placeholder="MCP ?ÑÍµ¨ ?†ÌÉù"
                    displayMode="count"
                    showBulkActions
                    className="flex-1 min-w-0"
                  />
              ...
              <div className="grid gap-2">
                <label className="text-sm font-medium text-slate-900">?πÏÇ¨?¥Ìä∏</label>
                <input
                  value={website}
              ...
              <div className="grid gap-2">
                <label className="text-sm font-medium text-slate-900">Ï£ºÏöî Î™©Ìëú *</label>
                <textarea
                  value={goal}
                  onChange={(e) => setGoal(e.target.value)}
                  placeholder="?êÏù¥?ÑÌä∏ Î™©ÌëúÎ•??ÖÎ†•?òÏÑ∏??"
        ...
        <Card className="mt-6 p-6">
          <div className="text-sm font-semibold text-slate-900">Î≤ÑÏ†Ñ Î™©Î°ù</div>
          <p className="mt-1 text-xs text-slate-500">
            Î∞∞Ìè¨ ?ÅÌÉú??Î≤ÑÏ†Ñ Î™©Î°ù?êÏÑúÎß?Í¥ÄÎ¶¨Îê©?àÎã§. Î∞∞Ìè¨Î•?ÏºúÎ©¥ ?¥Îãπ Î≤ÑÏ†ÑÎß??úÏÑ±?îÎê©?àÎã§.
          </p>
          <ul className="mt-3 grid grid-cols-[70px_80px_70px_70px_70px_70px_70px_120px_minmax(0,1fr)_44px] gap-x-[6px] divide-y divide-slate-200">
            ...
            {versionItems.length === 0 ? (
              <li className="col-span-full py-3 text-sm text-slate-500">Î≤ÑÏ†Ñ Í∏∞Î°ù???ÜÏäµ?àÎã§.</li>
            ) : (
