...
export function useConversationPageController(pageKey: ConversationPageKey = "/app/conversation") {
  ...
  const conversationActions = useConversationActions<ChatMessage, ModelState>({
    models,
    updateModel,
  ...
  const handleCopyTranscript = async (id: string) =>
    conversationActions.copyConversation(id, pageFeatures.adminPanel.copyConversation, conversationDebugOptions);

  const handleCopyIssueTranscript = async (id: string) =>
    conversationActions.copyIssue(id, pageFeatures.adminPanel.copyIssue);
  ...
  return {
    ...
    submitMessage: conversationActions.submitMessage,
    expandModelLayout,
    collapseModelLayout,
