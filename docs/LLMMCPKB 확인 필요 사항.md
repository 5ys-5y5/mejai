...
- **OpenAI (주력 응답 생성)**: `gpt-4.1-mini`
    - 고성능 대비 비용/지연 균형이 좋아 “운영 기본 모델”로 무난합니다.
...
> 운영 권장: “검색 품질”이 목표면 임베딩은 large/3072 쪽이 유리하지만, 비용/저장소/속도 때문에 1536로 시작하는 팀이 많습니다.
> 
...
- PDF/HTML은 “문단 단위 + 헤더 유지”가 검색 품질에 유리
...
- 마스킹:
    ...
    - 로그는 request/response를 “요약 로그”로 분리(원문 저장 금지 또는 암호화 저장)
- 레이트리밋:
...
- 2회 이후에도 불명확하면:
    - “제가 답을 확정할 수 없는 이유” + “필요한 정보 체크리스트”를 제시하고 **에스컬레이션** 또는 “답변 보류” 처리
