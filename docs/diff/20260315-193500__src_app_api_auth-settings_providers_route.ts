...
import {
  ...
  serializeProviderStateForResponse,
  upsertDefaultProviderConnection,
  writeSupportedProviderState,
...
export async function GET(req: NextRequest) {
  ...
  if (isSupportedProviderKey(provider)) {
    const state = readSupportedProviderState(providers, provider);
    return NextResponse.json({
      provider: serializeProviderStateForResponse(provider, state),
    });
  }
...
export async function POST(req: NextRequest) {
  ...
  let data:
    | { id: string; providers: Record<string, unknown> | null; user_id?: string }
    | null = null;
  ...
  if (isSupportedProviderKey(provider)) {
    const currentProviders = (data?.providers || {}) as Record<string, unknown>;
    const currentState = readSupportedProviderState(currentProviders, provider);
    let nextState = currentState;

    try {
      if (body.mode) {
        if ((body.mode === "create_connection" || body.mode === "update_connection") && !body.values) {
          return NextResponse.json({ error: "VALUES_REQUIRED" }, { status: 400 });
        }
        nextState = applyProviderConnectionCommand(
          provider,
          currentState,
          {
            mode: body.mode,
        ...
        nextState = upsertDefaultProviderConnection(provider, currentState, body.values, nowIso);
      }
    } catch (commandError) {
      return NextResponse.json(
        { error: commandError instanceof Error ? commandError.message : "INVALID_PROVIDER_COMMAND" },
        { status: 400 }
      );
    }

    const nextProviders = writeSupportedProviderState(currentProviders, provider, nextState);
    if (!data) {
      ...
      const { error: updateError } = await supabase
        .from("A_iam_auth_settings")
        .update({ providers: nextProviders, updated_at: nowIso })
        .eq("id", data.id);
      if (updateError) {
