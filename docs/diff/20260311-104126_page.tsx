...
export default function ConversationWidgetPage() {
  ...
  const [saveBarRect, setSaveBarRect] = useState<{ left: number; width: number } | null>(null);
  ...
  const hasUnsavedChanges = isDirty || policyDirty;
  const showSaveBar = (draft && hasUnsavedChanges) || saveSyncing;
  const isSaving = policySaving || policyLoading;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setInstallCopied(false), 1600);
  ...
  const previewTabStatus = useMemo(() => {
    ...
    return {
      chat: {
        enabled: resolved?.widget?.tabBar?.chat ?? false,
        visibility: resolved?.visibility?.widget?.tabBar?.chat ?? "admin",
      },
      list: {
        enabled: resolved?.widget?.tabBar?.list ?? false,
        visibility: resolved?.visibility?.widget?.tabBar?.list ?? "admin",
      },
      policy: {
        enabled: resolved?.widget?.tabBar?.policy ?? false,
        visibility: resolved?.visibility?.widget?.tabBar?.policy ?? "admin",
      },
      login: {
        enabled: resolved?.widget?.tabBar?.login ?? false,
        visibility: resolved?.visibility?.widget?.tabBar?.login ?? "admin",
      },
    } as const;
  }, [policyValue]);

  const launcherConfig = useMemo(() => {
    const launcher = policyValue?.widget?.launcher;
  ...
  const handleLauncherClick = useCallback(() => {
    ...
    setLauncherPreviewOpen((prev) => !prev);
  ...
  const handlePolicySave = useCallback(async () => {
    ...
    try {
      ...
      setPolicyLoading(true);
      const latestPolicy = await apiFetch<{ provider?: ConversationFeaturesProviderShape | null }>(
        `/api/widget-templates/${policyTemplateId}/chat-policy`
      );
      setPolicyValue(latestPolicy.provider || null);
      setPolicyBaselineFingerprint(buildPolicyFingerprint(latestPolicy.provider || null));
    } finally {
      setPolicyLoading(false);
      ...
      <div ref={contentRef} className="mx-auto w-full max-w-6xl">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4">
          ...
          <div className="space-y-6">
                ...
                {policyLoading ? (
                  <Card className="p-4 text-xs text-slate-500">정책을 불러오는 중...</Card>
                    ...
                    <ChatSettingsPanel
                      ...
                      inlineKbSampleOptions={inlineKbSampleOptions}
            ...
            <Card className="p-4 space-y-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    handleCopy(installScript);
                    setInstallCopied(true);
                  }}
                >
                  {installCopied ? "복사됨" : "설치 코드 복사"}
                </Button>
              </div>
              ...
              <div
                tabIndex={0}
                className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs font-mono text-slate-700 whitespace-pre-wrap select-text"
              >
            ...
            {draft ? (
              <div className="space-y-4">
                <Card className="p-4 space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {templatePreviewUrl ? (
                      <button
                        type="button"
                        onClick={() => window.open(templatePreviewUrl, "_blank", "noreferrer")}
                ...
                <Card className="p-4 space-y-3">
                  ...
                  <div className="text-[11px] text-slate-500">가로 폭을 드래그하여 변경할 수 있습니다.</div>
                  <div className="w-full">
                    <div
                      className="relative h-[260px] max-w-full overflow-hidden rounded-xl border border-slate-200 bg-slate-50"
        ...
        {showSaveBar ? (
          <div
            ...
            style={{
              left: saveBarRect?.left ?? 0,
              width: saveBarRect?.width ?? "auto",
            }}
          >
            <Button
              type="button"
              onClick={() => void handlePolicySave()}
              disabled={!policyTemplateId || isSaving}
              aria-label="저장하기"
              className={`h-12 w-full rounded-2xl border pointer-events-auto ${
                isSaving
                  ? "border-slate-300 bg-slate-300 text-slate-600 cursor-not-allowed"
                  : "border-slate-900 bg-slate-900 text-white hover:bg-slate-800"
              } !opacity-100 disabled:!opacity-100 disabled:bg-slate-300 disabled:text-slate-600`}
              style={
                isSaving
                  ? { opacity: 1, backgroundColor: "#d1d5db", color: "#475569" }
                  : { opacity: 1 }
              }
            >
              {isSaving ? "저장중" : "저장하기"}
