...
- MCP 테스트 결과:
  - `supabase` MCP: `B_chat_widgets`에서 `c9ab5088-1d28-4f7f-88f4-01c46fa9ddfc` 조회됨 (is_active=true, is_public=true)
  - `chrome-devtools` MCP: `/logindemo` 접속, 위젯 iframe 로드 확인
- 기능 검증 결과:
  - 로그인 폼 렌더: 정상
  - 위젯 로드: 정상
  - 위젯 메시지 송수신: 미검증 (사용자 입력 필요)
