...
import { normalizeAgentMcpBindings } from "@/lib/agentMcpBindings";
...
export async function POST(req: NextRequest) {
  ...
  if (body.agent_id) {
    const { data: agentRow } = await context.supabase
      .from("B_bot_agents")
      .select("mcp_provider_bindings")
      .eq("id", String(body.agent_id || "").trim())
  ...
  const { data: authSettingsRow } = await context.supabase
    .from("A_iam_auth_settings")
    .select("providers")
    .eq("org_id", context.orgId)
    .eq("user_id", context.user.id)
    .maybeSingle();
  authSettingsProviders = (authSettingsRow?.providers || {}) as Record<string, unknown>;
