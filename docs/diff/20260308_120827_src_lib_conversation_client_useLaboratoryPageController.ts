...
export function useLaboratoryPageController(pageKey: ConversationPageKey = "/app/laboratory") {
  ...
  useEffect(() => {
    setModels((prev) => {
      let changed = false;
      const next = prev.map((model) => {
        if (visibleLlmIds.includes(model.config.llm as "chatgpt" | "gemini")) return model;
        changed = true;
        return { ...model, config: { ...model.config, llm: effectiveDefaultLlm } };
      });
      return changed ? next : prev;
    });
  ...
  useEffect(() => {
    ...
    setModels((prev) => {
      let changed = false;
      const next = prev.map((model) => {
        if (model.config.inlineKb === "" && model.config.inlineKbSampleSelectionOrder.length === 0) {
          return model;
        }
        changed = true;
        return {
  ...
  useEffect(() => {
    ...
    setModels((prev) => {
      let changed = false;
      const next = prev.map((model) => {
        const nextKbId = model.config.kbId || firstUserKb || "";
        if (model.config.kbId === nextKbId) return model;
        changed = true;
        return {
          ...model,
          config: { ...model.config, kbId: nextKbId },
        };
      });
      return changed ? next : prev;
    });
