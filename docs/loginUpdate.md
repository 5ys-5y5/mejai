...
- `src/lib/conversation/client/useLaboratoryPageController.ts` (빌드 에러 해소 목적의 승인된 예외 수정)
- `src/lib/conversation/pageFeaturePolicy.ts` (위젯 초기 탭 정책 추가)
- `src/components/conversation/ChatSettingsPanel.tsx` (정책 UI에 초기 탭 설정 추가)
- `src/app/embed/[key]/page.tsx` (위젯 초기 탭 적용 로직)
...
- 기능 검증 결과 (위젯 로드): 정상
- 기능 검증 결과 (위젯 메시지 송수신): 미검증 (사용자 입력 필요)
## 추가 실행 기록 (2026-03-08 - 변경 요청 반영)
- 변경 사항: `/logindemo` 위젯 래퍼에 테두리 적용, 높이 고정값 300px로 변경
- MCP 테스트 결과 (supabase): `B_chat_widgets`에서 `c9ab5088-1d28-4f7f-88f4-01c46fa9ddfc` 조회됨 (is_active=true, is_public=true)
- MCP 테스트 결과 (chrome-devtools): `/logindemo` 접속, 위젯 iframe 로드 확인 (테두리/높이 변경 반영)
- 기능 검증 결과 (위젯 로드): 정상
## 추가 실행 기록 (2026-03-08 - 로그인 라벨 정렬 변경)
- 변경 사항: 이메일/비밀번호 라벨 오른쪽에 회원가입/비밀번호 찾기 링크 배치
- 빌드: `npm run build` 성공
- MCP 테스트 결과 (supabase): `B_chat_widgets`에서 `c9ab5088-1d28-4f7f-88f4-01c46fa9ddfc` 조회됨 (is_active=true, is_public=true)
- MCP 테스트 결과 (chrome-devtools): `/logindemo` 접속, 라벨 우측 링크 배치 확인
- 기능 검증 결과 (로그인 폼 렌더): 정상
## 추가 실행 기록 (2026-03-08 - 로그인다이얼로그 단순화 및 위젯 추가)
- 변경 사항: 로고/타이틀/헬퍼 텍스트 제거, 컨텐츠 상단 마진 제거, 위젯 2개 배치(상단 정책 탭 노출)
- 빌드: `npm run build` 성공
- MCP 테스트 결과 (supabase): `B_chat_widgets`에서 `c9ab5088-1d28-4f7f-88f4-01c46fa9ddfc` 조회됨 (is_active=true, is_public=true)
- MCP 테스트 결과 (chrome-devtools): `/logindemo` 접속, 2개 위젯 렌더 및 상단 위젯 정책 탭 파라미터 적용 확인
## 추가 실행 기록 (2026-03-08 - 위젯 초기 탭 정책 추가)
- 변경 사항: 위젯 기본 탭 설정(`widget.defaultTab`) 추가 및 정책 UI에 선택 항목 노출, 임베드 초기 탭 적용 로직 추가
- 빌드: `npm run build` 성공 (1회 타임아웃 후 재시도 성공)
- MCP 테스트 결과 (supabase): `B_chat_widgets`에서 `c9ab5088-1d28-4f7f-88f4-01c46fa9ddfc` 조회됨
- MCP 테스트 결과 (chrome-devtools): `/logindemo` 접속, 상단 위젯 `tab=policy` 로드 및 하단 위젯 기본 탭 로드 확인
