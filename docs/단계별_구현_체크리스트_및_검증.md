...
- [x] 에이전트/KB 버전 정책 정비 (Owner: LLM)
  ...
  - [x] 적용 파일: `src/app/api/agents/[id]/route.ts`, `src/app/api/kb/[id]/route.ts`, `src/app/api/playground/chat/route.ts`
...
- [x] `llm_mk2.ts` 분리
- [x] mk2 API 엔드포인트 노출 (`/api/playground/chat_mk2`)

### 5) 비교 UI
- [x] 비교 페이지 추가 (`/app/agents/[id]/playground-compare`)
...
- 사용자의 말투를 학습해서 답변할 것
- 주소지 선택하고 반영할 수 있도록
- debug_cafe24_order로 주문 정보, 배송 정보 습득하게 할 것, 현재 lookup_order는 불완전한 mcp임
