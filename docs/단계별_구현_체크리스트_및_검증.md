...
- [x] RLS 정책 설계 및 적용
- [x] 에이전트/KB 버전 정책 정비 (Owner: LLM)
  - [x] 에이전트 버전 생성 기준을 LLM/MCP/KB 변경으로 제한 (메타 변경은 동일 버전 업데이트)
  - [x] 에이전트 PATCH 시 무변경 요청으로 전 버전 비활성화되는 버그 제거
  - [x] KB 내용 변경 시 에이전트 자동 갱신 제거 → 선택한 에이전트만 새 버전 생성
  - [x] 에이전트가 과거 KB 버전을 사용해도 동작 가능하도록 검증 흐름 수정
  - [x] 적용 파일: `src/app/api/agents/[id]/route.ts`, `src/app/api/kb/[id]/route.ts`, `src/app/api/runtime/chat/legacy/route.ts`
- [x] DB 제약/인덱스 보강 (Owner: LLM)
  - [x] `agent.parent_id` self FK 추가
  - [x] `agent` 단일 활성 버전 유니크 인덱스(`agent_one_active`)
  - [x] `knowledge_base` 단일 활성 버전 유니크 인덱스 재확인/보강(`knowledge_base_one_active`)
...
- [ ] 프론트엔드 UI와 백엔드 API 연동
  ...
  - [x] KB 버전 변경 선택/알림 UI 보강
    - [x] KB 수정 화면에서 업데이트 적용 에이전트 선택 UI 추가
    - [x] 에이전트 목록에서 KB 업데이트 알림 표시
    - [x] 에이전트 설정 화면에서 최신 KB 빠른 선택 제공
    - [x] 적용 파일: `src/app/app/kb/[id]/page.tsx`, `src/app/app/agents/page.tsx`, `src/app/app/agents/[id]/page.tsx`
...
## Phase 3.5: Runtime LLM/MCP/KB + Cafe24 주문 조치 테스트 (신규)
...
-- [x] WS 서버 구현 (Owner: LLM)
  ...
  - [ ] Runtime → WS 연결 → 메시지 송수신
  - [ ] LLM 스트리밍 토큰을 WS로 전달

### 4) KB RAG 연동
- [ ] 활성 KB 조회: `knowledge_base.is_active = true` (Owner: LLM)
  - [x] 에이전트가 선택한 KB 버전 그대로 사용 (활성 여부와 무관) — 운영 정책 반영
...
### 6) Runtime 통합 플로우
- [ ] Runtime에서 agent 선택 → LLM/MCP/KB로 실제 응답 생성 (Owner: LLM)
...
- [ ] Runtime에서 실제 LLM 응답 스트리밍 확인
- [ ] Cafe24 order_id 찾기 및 허용 조치 실행 확인
- [ ] 모든 단계가 `bot_context`로 추적 가능

---

## Phase 3.6: MK2 재입고 정책/도구/비교 페이지 (진행 중)

### 1) 스키마/정책 정의
- [x] mk2 최소 스키마 SQL 초안 작성 (`docs/mk2/001_schema_restock.sql`)
- [x] mk2 정책 팩 초안 작성 (`docs/mk2/policy_pack_mk2.json`, `docs/mk2/policy_pack_mk2.md`)
- [x] policy_pack 템플릿 문구 한국어화

### 2) 정책 엔진/결정 단계
- [x] product.answerable/product.restock_known predicate 추가
- [x] 정책 args 템플릿 변수 해석(`{{product.id}}`, `{{entity.channel}}`, `{{entity.phone}}`) 구현
- [x] mk2 decision JSON을 LLM 입력에 포함
- [x] 입력 단계에서 intent를 정책으로 결정 (text.restock_inquiry/subscription)

### 3) MCP 도구/정책
- [x] mk2 도구 스텁 추가 (`resolve_product`, `read_product`, `read_supply`, `subscribe_restock`, `trigger_restock`)
- [x] mk2 도구 스키마 SQL 초안 작성 (`docs/mk2/002_mcp_tools_restock.sql`)
- [x] org별 mcp_tool_policies SQL 작성 (`docs/mk2/003_mcp_tool_policies_restock.sql`)
- [ ] mcp_tools/mcp_tool_policies 실제 DB 적용 확인

### 4) 라우트/LLM 분리
- [x] `route_mk2.ts` 생성 및 mk2 전용 흐름으로 단순화
- [x] `llm_mk2.ts` 분리
- [x] mk2 API 엔드포인트 노출 (`/api/runtime/chat/shipping`)

### 5) 비교 UI
- [x] 비교 페이지 추가 (`/app/labolatory?agentId=[id]`)
- [x] 에이전트 설정 화면에 비교 테스트 링크 추가
- [x] 비교 페이지에 세션 상세 보기 링크 추가

### 6) 운영 반영/검증
- [x] policy_pack content_json 최신 반영 확인
- [x] 정책 적용/검증 SQL 최신화 (`docs/mk2/004_apply_policy_pack_mk2.sql`, `docs/mk2/005_verify_policy_pack_mk2.sql`)
- [ ] mk2 정책 기반 intent/강제응답 시나리오 테스트
- [ ] 재입고 알림 구독 end-to-end 확인 (subscribe_restock → restock_subscription 저장)

### 7) 다음 진행(사용자 요청: 1,2,3 모두 실행)
- [ ] Cafe24 상품/재고/입고 MCP 실API 연결 (`read_product`, `read_supply`, `read_shipping`)
  - [x] Cafe24 상품 조회 어댑터 구현 (mall.read_product)
  - [x] Cafe24 공급(공급사) 조회 어댑터 구현 (mall.read_supply)
  - [x] Cafe24 배송 설정 조회 어댑터 구현 (mall.read_shipping)
  - [x] mcp_tools/mcp_tool_policies에 실API 도구 등록/허용 확인
- [ ] 재입고 이벤트 워커 설계 + 알림/주문 파이프라인 초안
  - [x] 재입고 트리거 정의(재고>0/판매상태 변경)
  - [x] 구독 데이터 모델 확정 및 상태 전이 설계
  - [ ] 알림 채널(문자/카카오/이메일) 발송 인터페이스 설계
  - [x] 주문 실행 2단계 확인 플로우 설계
  - [x] 파이프라인 초안 문서화 (`docs/mk2/006_restock_workflow.md`)
- [ ] 범용 intent 확장(FAQ/배송/반품 등) 정책팩 설계
  - [x] intent 분류 규칙 추가 (input stage)
  - [x] intent별 tool allowlist/denylist 추가 (tool stage)
  - [x] 배송/환불 intent MCP 연동 룰 추가 (lookup_order/track_shipment/create_ticket)
  - [x] 주문 변경 intent + 배송지 변경(write) 룰 추가
  - [x] 템플릿/문구 확장 (output stage)
  - [x] 주문번호 누락 시 안내 템플릿 추가 (shipping/refund)
  - [x] 배송/환불/FAQ 출력 포맷 고정 (format_output)
  - [x] 휴대폰 기반 주문 조회(list_orders) 정책 추가
  - [x] list_orders 결과 기반 주문번호 선택 안내 추가
  - [x] 주문/배송 MCP 결과 요약 포맷 추가
  - [x] 초안 문서화 (`docs/mk2/007_intent_policy_pack.md`)
- [ ] 주문번호 선택(후속 응답) 플로우
  - [ ] 이전 턴의 order_choices를 이용한 번호 선택 처리
  - [ ] 후속 질문에서 intent 미분류 시 이전 intent 재사용
  - [ ] 선택된 order_id를 entity.order_id로 주입해 policy 실행
 - [ ] 주문 배송지 변경(write) 도구 연결
   - [x] MCP 도구 스키마 추가 (`docs/mk2/009_mcp_tools_order_update.sql`)
   - [x] Cafe24 어댑터 구현 (`update_order_shipping_address`)
   - [x] 정책팩 룰 추가 (intent/order_change + force_tool_call)
   - [ ] mcp_tools/mcp_tool_policies 실제 DB 적용 확인
   - [ ] 주문 변경 성공/실패 테스트 시나리오 검증

### 8) 사용자 답변 반영(운영 방향 요약)
- [x] KB는 텍스트만으로도 답변 가능해야 함 → 정책/판정 단계는 “KB에 없으면 모른다” 원칙 유지
- [x] Cafe24 전체 scope 사용 전제 → MCP 도구 카탈로그 확장 필요(정책 매트릭스 유지)
- [x] 알림/주문 플로우는 하드코딩 없이 MCP + 정책으로 동작해야 함
- [ ] 주문/환불/배송 intent의 실제 MCP 호출 결과 검증(lookup_order/track_shipment/create_ticket)
