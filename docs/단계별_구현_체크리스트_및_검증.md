...
- [ ] 주문 조치 범위 확정 (Owner: USER)
  - 기본 가능: `lookup_order`, `track_shipment`, `create_ticket`
  - 추가 실행 액션(주문 상태 변경/환불/취소 등)은 스코프/엔드포인트 확인 후 확정
- [ ] LLM SDK/WS 패키지 설치 승인 (Owner: USER)
  - 필요 패키지: `openai`, `@google/generative-ai`, `ws`
- [ ] KB 임베딩 생성 파이프라인 필요 여부 확정 (Owner: USER)
...
- [ ] OpenAI SDK 연동 (Owner: LLM)
  - [ ] `OPENAI_API_KEY` 환경 변수 사용
  - [ ] `gpt-4.1-mini`, `gpt-4.1` 라우팅 구현
- [ ] Gemini SDK 연동 (Owner: LLM)
  - [ ] `GEMINI_API_KEY` 환경 변수 사용
  - [ ] `gemini-2.5-flash-lite`, `gemini-2.5-pro` 라우팅 구현
- [ ] 공통 응답 포맷/에러 핸들링 정의 (Owner: LLM)
